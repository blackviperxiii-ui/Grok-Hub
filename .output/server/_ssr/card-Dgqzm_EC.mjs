...
import { r as computeNextRun } from "./automation-schedule-fGYqzfa1.mjs";
...
var SETUP_SYNC_KIND = "grokhub-setup-v1";
var GIST_DESCRIPTION = "GrokHub setup sync (do not delete) — auto-managed";
function accountKey(oauth) {
	if (oauth.email) return oauth.email.toLowerCase().trim();
	if (oauth.idToken) try {
	...
	return {
		...
		setup: {
			...
			openClawWorkspace: input.openClawWorkspace ? {
				...
				skillCount: Array.isArray(input.openClawWorkspace.files) ? input.openClawWorkspace.files.length : void 0
			} : null
		}
	};
}
function parseSetupPack(raw) {
	if (!raw || typeof raw !== "object") return null;
	const o = raw;
	if (o.kind !== "grokhub-setup-v1" || !o.setup || typeof o.setup !== "object") return null;
	return o;
}
/** Merge pack into store-shaped patch pieces (caller applies). */
function mergeSetupPack(pack, current, opts) {
	const applied = [];
	const s = pack.setup;
	const out = { applied };
	if (s.mode) {
	...
	if (s.skills?.length) {
		if (opts?.replaceSkills) out.skills = s.skills;
		else {
	...
	if (s.automations?.length) {
		if (opts?.replaceAutomations) out.automations = s.automations;
		else {
	...
	if (s.connectors?.length) {
		...
		for (const c of s.connectors) {
			...
			byId.set(c.id, {
				...
				status: prev?.status === "connected" ? "connected" : c.status === "connected" ? "connected" : prev?.status || "disconnected"
			});
		}
...
async function findSetupGist(token, account) {
	const res = await ghFetch("/gists?per_page=100", token);
	if (!res.ok) return null;
	const list = await res.json();
	const needle = `${GIST_DESCRIPTION} · ${account}`;
	const hit = list.find((g) => (g.description || "").includes(needle)) || list.find((g) => (g.description || "").includes("GrokHub setup sync (do not delete) — auto-managed") && Object.keys(g.files || {}).some((f) => f.includes("grokhub-setup")));
	return hit ? {
		id: hit.id,
		updatedAt: hit.updated_at
	} : null;
}
async function pushSetupToGist(token, pack, gistId, rawContent) {
	...
	try {
		let res;
		if (gistId) res = await ghFetch(`/gists/${gistId}`, token, {
			method: "PATCH",
			body: JSON.stringify(body)
		});
		else {
			const existing = await findSetupGist(token, account);
			if (existing?.id) res = await ghFetch(`/gists/${existing.id}`, token, {
				method: "PATCH",
				body: JSON.stringify(body)
			});
			else res = await ghFetch("/gists", token, {
				method: "POST",
				body: JSON.stringify(body)
			});
		}
		if (!res.ok) {
			...
			return {
				ok: false,
				error: `GitHub ${res.status}: ${t.slice(0, 200)}`
			};
		}
		...
		return {
			ok: false,
			error: e instanceof Error ? e.message : "gist push failed"
		};
	}
}
async function pullSetupFromGist(token, account, gistId) {
	try {
		let id = gistId;
		if (!id) id = (await findSetupGist(token, account))?.id;
		if (!id) return {
			ok: false,
			error: "No setup gist found for this Grok account yet — push from another device first"
		};
		...
		const file = files.find((f) => (f.filename || "").includes("grokhub-setup")) || files[0];
		if (!file?.content) return {
			ok: false,
			error: "Gist has no setup file"
		};
		const raw = file.content;
		try {
			const pack = parseSetupPack(JSON.parse(raw));
			if (pack) return {
				ok: true,
				pack,
				raw,
			...
			return {
				ok: false,
				error: "Invalid setup file in gist"
			};
		}
	} catch (e) {
		return {
			ok: false,
			error: e instanceof Error ? e.message : "gist pull failed"
		};
	}
}
var MAX_HITS = 80;
...
var useGrokHub = create()(persist((set, get) => ({
	...
	hydrateSecrets: async () => {
		try {
			...
			if (!patch.ssoCookie && typeof window !== "undefined" && window.grokhubDesktop?.grok?.getWebsiteSso) try {
				const sso = await window.grokhubDesktop.grok.getWebsiteSso();
				if (sso?.cookie) {
					patch.ssoCookie = sso.cookie;
					import("./secrets-client-By5XiP9R.mjs").then((m) => m.secretsSet("ssoCookie", sso.cookie));
				}
			} catch {}
	...
	setSsoCookie: (cookie) => {
		...
		const normalized = raw && !raw.includes("=") ? `sso=${raw}` : raw;
		set((s) => ({
			ssoCookie: normalized,
			grokConnected: normalized ? s.grokConnected === true ? true : s.oauth || s.apiKey ? s.grokConnected : true : s.grokConnected,
			grokStatusDetail: normalized ? s.oauth || s.apiKey ? s.grokStatusDetail : "Free Grok · website session linked" : s.grokStatusDetail,
			usage: normalized && s.usage.plan !== "free" && !s.oauth && !s.apiKey ? {
				...s.usage,
				plan: "free"
			} : s.usage
		}));
		import("./secrets-client-By5XiP9R.mjs").then((m) => m.secretsSet("ssoCookie", normalized));
		if (typeof window !== "undefined" && window.grokhubDesktop?.grok?.injectWebsiteCookie) window.grokhubDesktop.grok.injectWebsiteCookie(normalized);
		get().refreshUsage();
	...
	setSetupSyncMeta: (patch) => {
		set((st) => ({ setupSyncMeta: {
			...st.setupSyncMeta,
			...patch
		} }));
	},
	scheduleSetupAutoPush: () => {
		if (!get().setupSyncMeta?.autoPushOnChange || !get().oauth?.accessToken) return;
		const w = globalThis;
		if (typeof window !== "undefined" && w.__grokhubSetupPushTimer) window.clearTimeout(w.__grokhubSetupPushTimer);
	...
	exportSetupPackJson: async (opts) => {
		...
		if (opts?.passphrase?.trim()) {
			const { encryptSetupJson } = await import("./setup-crypto-bPN_Pa4r.mjs");
	...
	importSetupPackJson: async (json, opts) => {
		try {
			...
			return {
				ok: true,
				detail: `Applied: ${merged.applied.join(", ") || "nothing"}`
			};
		} catch (e) {
			return {
				ok: false,
	...
	pushSetupSync: async (opts) => {
		const s = get();
		if (!s.oauth?.accessToken) return {
			ok: false,
			detail: "Sign in with Grok OAuth first"
		};
		const pack = buildSetupPack({
			oauth: s.oauth,
			mode: s.mode,
		...
		try {
			const key = `setup-pack:${accountKey(s.oauth)}`;
			if (typeof window !== "undefined" && window.grokhubDesktop?.state?.set) await window.grokhubDesktop.state.set(key, storeBody);
		} catch {}
		...
		if (!gh) {
			set((st) => ({ setupSyncMeta: {
				...st.setupSyncMeta,
				lastPushAt: Date.now(),
				lastAccount: accountKey(s.oauth),
				lastDetail: effectivePass ? "Saved encrypted local vault (add GitHub token for cloud)" : "Saved local account vault (add GitHub token for cloud sync)"
			} }));
			return {
				ok: true,
				detail: effectivePass ? "Encrypted setup saved locally. Link a GitHub token for cross-device Gist sync." : "Setup saved for this Grok account locally. Link a GitHub token in Settings to sync across machines via private Gist."
			};
		}
		const r = await pushSetupToGist(gh, pack, s.setupSyncMeta?.lastGistId, effectivePass ? storeBody : void 0);
		set((st) => ({ setupSyncMeta: {
			...st.setupSyncMeta,
			lastPushAt: Date.now(),
			lastGistId: r.gistId || st.setupSyncMeta?.lastGistId,
			lastAccount: accountKey(s.oauth),
			lastDetail: r.ok ? effectivePass ? "Pushed encrypted pack to GitHub Gist" : "Pushed to GitHub Gist" : r.error
		} }));
		get().pushActivity({
			kind: "auth",
		...
		return {
			ok: Boolean(r.ok),
			detail: r.ok ? `Pushed setup for ${pack.account.email || pack.account.sub}` : r.error || "push failed"
		};
	},
	pullSetupSync: async (opts) => {
		...
		if (gh) {
			...
			if (r.ok && (r.pack || r.raw)) {
				if (r.pack) pack = r.pack;
				else if (r.raw) try {
					const { unwrapSetupPayload } = await import("./setup-crypto-bPN_Pa4r.mjs");
					const plain = await unwrapSetupPayload(r.raw, opts?.passphrase);
					pack = parseSetupPack(JSON.parse(plain));
				} catch (e) {
					detail = e instanceof Error ? e.message : "Could not decrypt Gist pack";
				}
				if (pack) detail = "Pulled from GitHub Gist";
				if (r.gistId) set((st) => ({ setupSyncMeta: {
					...st.setupSyncMeta,
					lastGistId: r.gistId
				} }));
			} else detail = r.error || "No gist";
		}
		if (!pack && typeof window !== "undefined" && window.grokhubDesktop?.state?.get) try {
			...
			if (got?.value) {
				try {
					...
					detail = e instanceof Error ? e.message : "Could not open local vault (wrong passphrase?)";
				}
				if (pack) detail = detail ? `${detail}; local vault` : "Loaded local account vault";
			}
		} catch {}
		if (!pack) return {
			ok: false,
			detail: detail || "No setup found for this Grok account yet. Push from a configured machine, or import a pack file."
		};
		const merged = mergeSetupPack(pack, {
			agents: s.agents,
			skills: s.skills,
	...
	syncSetupWithGrokAccount: async (opts) => {
		const s = get();
		if (!s.oauth?.accessToken && !s.apiKey) return {
			ok: false,
			detail: "Connect Grok OAuth first"
		};
		const parts = [];
		try {
			await get().syncFromGrok({
	...
	linkGrokWebsiteSession: async () => {
		try {
			if (typeof window !== "undefined" && window.grokhubDesktop?.grok?.linkWebsiteSession) {
				...
				return {
					ok: false,
					detail: r?.error || "No session captured. Sign in until Grok chat loads, click “Use this session” in the bar, or paste sso= from browser cookies."
				};
			}
	...
	toggleAutomation: (id) => {
		set((s) => ({ automations: s.automations.map((a) => {
			if (a.id !== id) return a;
			...
			return {
				...a,
				enabled,
				nextRun: computeNextRun(a.schedule, a.time, Date.now(), a.lastRun, a.times, a.heartbeatEveryMin)
			};
		}) }));
	},
	runAutomation: async (id) => {
		...
		const { computeNextRun } = await import("./automation-schedule-fGYqzfa1.mjs").then((n) => n.n).then((n) => n.n);
		set((s) => ({
			running: false,
			streamStatus: null,
			automations: s.automations.map((a) => a.id === id ? {
				...
				nextRun: a.schedule === "once" ? void 0 : computeNextRun(a.schedule, a.time, Date.now(), Date.now(), a.times, a.heartbeatEveryMin),
	...
	tickAutomations: async (opts) => {
		const { dueAutomations, dueHeartbeatAutomations, ensureAutomationSchedule } = await import("./automation-schedule-fGYqzfa1.mjs").then((n) => n.n).then((n) => n.n);
		const now = Date.now();
		set((s) => ({
		...
		const due = opts?.heartbeatOnly ? dueHeartbeatAutomations(list, now) : dueAutomations(list, now);
	...
	addAutomation: (input) => {
		const times = (input.times && input.times.length ? input.times : [input.time || "09:00"]).map((x) => String(x).trim()).filter(Boolean);
		...
		const auto = {
			...
			skillIds: [],
			runCount: 0,
			nextRun: computeNextRun(input.schedule, primary, Date.now(), void 0, unique, input.heartbeatEveryMin ?? 5)
		};
		...
		get().pushActivity({
			...
			detail: auto.schedule === "heartbeat" ? `heartbeat every ${auto.heartbeatEveryMin || 5}m` : `${auto.schedule} @ ${unique.join(", ")}`,
			status: "success"
		});
	},
	stopChat: () => {
	...
	sendChat: async (text) => {
		...
		let finalAnswer = "";
		let aborted = false;
		const { extractHostCommands, stripHostCommands, inferHostCommandsFromUser } = await import("./grok-BeJNSAgB.mjs");
		const { extractConnectorCommands, stripConnectorCommands, runConnectorTool } = await import("./connector-tools-C-fkJiLT.mjs");
		const { extractSelfModCommands, stripSelfModCommands, selfModList, selfModRead, selfModWrite, selfModPatch, selfModSnapshot } = await import("./self-mod-client-CTO9n7NH.mjs");
		const scrubAssistant = (s) => stripSelfModCommands(stripConnectorCommands(stripHostCommands(s)));
		try {
			if (isLocalSlash) {
				...
				while (rounds < maxRounds) {
					...
					const stNow = get();
					const freeTier = stNow.usage.plan === "free" || !stNow.oauth?.accessToken && !stNow.apiKey;
					const result = await grokChatStream({
						messages: history,
						mode: routed,
						model: freeTier ? void 0 : modelId,
						apiKey: get().apiKey || void 0,
						...
						workspaceContext: [oc?.contextBundle || "", (await import("./grok-BeJNSAgB.mjs")).connectorContextBlock(get().connectors)].filter(Boolean).join("\n").slice(0, 28e3) || void 0
					}, {
						signal: abort.signal,
						onStatus: (st) => {
							if (gen !== chatGeneration) return;
					...
					if (result.ok && (result.content || roundText)) {
						usedLive = true;
						if (result.freeTier || String(result.accessPath || "").includes("free")) set((s) => ({
							grokConnected: true,
							grokStatusDetail: result.accessPath === "website_free" ? "Free Grok · website session" : result.fallbackFrom ? `Free Grok fallback · ${result.model || "mini"}` : "Free Grok · free-tier models",
							usage: s.oauth || s.apiKey ? s.usage : {
								...s.usage,
								plan: "free"
							}
						}));
						...
						if (selfCmds.length) if (!get().desktop.selfModifyEnabled) {
							history.push({
								role: "assistant",
								content: full
							});
							history.push({
								role: "user",
								content: "SELF_MOD_RESULT: blocked — self-modification is disabled. User must enable Settings → Desktop → Allow self-modification, or use Factory reinstall if the app is broken."
							});
							patchBot((visible || "") + "\n\n_Self-mod blocked (enable in Settings)._\n_Continuing…_", { streaming: true });
							if (!cmds.length) continue;
						} else {
							...
							for (const sc of selfCmds.slice(0, 4)) {
								...
								try {
									if (sc.kind === "list") {
										...
										outputs.push(`LIST ${sc.path}\n${JSON.stringify(r.entries || r, null, 2).slice(0, 4e3)}`);
									} else if (sc.kind === "read") {
										const r = await selfModRead(sc.path);
										outputs.push(r.ok ? `READ ${sc.path}\n${(r.content || "").slice(0, 8e3)}` : `READ ${sc.path} failed: ${r.error}`);
									} else if (sc.kind === "write") {
										const r = await selfModWrite(sc.path, sc.content, { note: "agent SELF_MOD" });
										outputs.push(r.ok ? `WRITE ${sc.path} ok` : `WRITE ${sc.path} failed: ${r.error}`);
										get().pushActivity({
											kind: "skill",
											title: `Self-mod write ${sc.path}`,
											detail: r.ok ? "ok" : String(r.error || "fail"),
											status: r.ok ? "success" : "failed"
										});
									} else if (sc.kind === "patch") {
										const r = await selfModPatch(sc.path, sc.find, sc.replace, { note: "agent SELF_MOD patch" });
										outputs.push(r.ok ? `PATCH ${sc.path} ok` : `PATCH ${sc.path} failed: ${r.error}`);
										get().pushActivity({
											kind: "skill",
										...
										outputs.push(r.ok ? `SNAPSHOT ${r.id} files=${r.fileCount}` : `SNAPSHOT failed`);
									}
								} catch (e) {
									outputs.push(`SELF_MOD error: ${e instanceof Error ? e.message : "failed"}`);
								}
							}
							if (aborted) break;
							history.push({
					...
					finalAnswer = [
						...
						err,
						"",
						hasOauth ? "Your OAuth session is saved. Try reconnecting OAuth, paste an xAI API key, or Link Grok website for free-tier chat." : "Fix: Link Grok website (free), Connect with Grok OAuth, or paste an xAI API key (console.x.ai free credits)."
					].join("\n");
					set({
						grokConnected: false,
	...
	tickHeartbeat: () => {
		...
		get().tickAutomations({ heartbeatOnly: true });
	},
	...
	migrate: (persisted) => {
		...
		if (desk) {
			...
			if (desk.selfModifyEnabled === void 0) desk.selfModifyEnabled = false;
			if (!s.setupSyncMeta) s.setupSyncMeta = {
				autoPullOnLogin: true,
				autoPushOnChange: false
			};
			if (s.preferFreeGrok === void 0) s.preferFreeGrok = true;
			if (Array.isArray(s.automations)) s.automations = s.automations.map((a) => {
				const times = Array.isArray(a.times) && a.times.length ? a.times : a.time ? [a.time] : ["09:00"];
				return {
					...a,
					times,
