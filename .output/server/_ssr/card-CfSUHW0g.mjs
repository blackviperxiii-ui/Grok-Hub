...
var useGrokHub = create()(persist((set, get) => ({
	...
	sendChat: async (text) => {
		...
		let finalAnswer = "";
		let aborted = false;
		const { extractHostCommands, stripHostCommands, inferHostCommandsFromUser } = await import("./grok-DjkVIlRC.mjs");
			...
			if (isLocalSlash) {
				...
				while (rounds < maxRounds) {
					...
					const result = await grokChatStream({
						...
						tokens: get().oauth,
						workspaceContext: [oc?.contextBundle || "", (await import("./grok-DjkVIlRC.mjs")).connectorContextBlock(get().connectors)].filter(Boolean).join("\n").slice(0, 28e3) || void 0
					}, {
						signal: abort.signal,
						onStatus: (st) => {
							if (gen !== chatGeneration) return;
