...
function catalogConnectors() {
	return [
		...
		},
		{
			...
			name: "Gmail & Calendar",
			category: "Google",
			description: "Planned connector — opens Google sign-in (not live tools yet).",
			status: "disconnected",
			tools: [
				"search_mail",
				"draft_reply",
		...
		},
		{
			...
			name: "Google Drive",
			category: "Google",
			description: "Planned connector — opens Drive (not live tools yet).",
			status: "disconnected",
			tools: [
				"search_files",
				"read_doc",
		...
		},
		{
			...
			name: "Notion",
			category: "Workspace",
			description: "Planned connector — opens Notion (not live tools yet).",
			status: "disconnected",
			tools: [
				"search_pages",
				"update_page",
		...
		},
		{
			...
			category: "Microsoft",
			description: "Planned connector — opens Outlook (not live tools yet).",
			status: "disconnected",
			tools: [
				"search_inbox",
				"draft_mail",
		...
		},
		{
			...
			category: "Microsoft",
			description: "Planned connector — opens Teams (not live tools yet).",
			status: "disconnected",
			tools: [
				"list_channels",
				"post_message",
		...
		},
		{
			...
			name: "Linear",
			category: "Projects",
			description: "Planned connector — opens Linear (not live tools yet).",
			status: "disconnected",
			tools: [
				"list_issues",
				"create_issue",
...
var useGrokHub = create()(persist((set, get) => ({
	...
	connectConnector: async (id) => {
		...
		get().pushActivity({
			...
			detail: "Live API wiring is not available yet. Only Grok, Desktop Host, and GitHub are fully integrated.",
			status: "queued"
		});
	},
	toggleSkill: (id) => {
	...
	sendChat: async (text) => {
		...
		let finalAnswer = "";
		let aborted = false;
		const { extractHostCommands, stripHostCommands, inferHostCommandsFromUser } = await import("./grok-BlMJRCyx.mjs");
		try {
			if (isLocalSlash) {
...
//#endregion
export { CardHeader as a, CardDescription as i, Card as n, CardTitle as o, CardContent as r, useGrokHub as s, Badge as t };
