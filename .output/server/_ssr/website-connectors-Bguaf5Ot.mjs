import { mapWebsiteConnectorName } from "./connector-tools-C-fkJiLT.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/website-connectors-Bguaf5Ot.js
/**
...
function cookieHeader(ssoCookie) {
...
function normalizeList(payload) {
	const hits = [];
	const push = (name, account, connected = true) => {
	...
	const walk = (node, depth = 0) => {
		...
		const o = node;
		const name = String(o.name || o.displayName || o.title || o.provider || o.appName || o.connectorName || "").trim();
		const status = String(o.status || o.state || o.connectionStatus || "").toLowerCase();
		const account = String(o.email || o.account || o.accountEmail || o.userEmail || o.username || o.login || "").trim();
		const connected = status.includes("connect") || o.connected === true || o.isConnected === true || o.installed === true || !status && Boolean(name);
		if (name && (connected || account)) push(name, account || null, connected || Boolean(account));
		for (const v of Object.values(o)) if (v && typeof v === "object") walk(v, depth + 1);
	};
	walk(payload);
	return hits;
}
function parseConnectorsHtml(html) {
	const hits = [];
	const names = [
		"GitHub",
		"Notion",
	...
	for (const name of names) {
		const re = new RegExp(`${name}[\\s\\S]{0,240}?(Connected|connected|[\\w.+-]+@[\\w.-]+)`, "i");
...
async function fetchWebsiteConnectors(opts) {
	if (typeof window !== "undefined" && window.grokhubDesktop?.grok?.websiteConnectors) try {
		const r = await window.grokhubDesktop.grok.websiteConnectors({
			ssoCookie: opts.ssoCookie,
			bearer: opts.bearer
		});
		if (r?.connectors?.length) return {
			ok: true,
			source: "electron",
			connectors: r.connectors.map((c) => ({
				...
				status: c.status === "disconnected" ? "disconnected" : "connected"
			})),
	...
	for (const url of REST_CANDIDATES) try {
