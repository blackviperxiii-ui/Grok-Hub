...
//#region node_modules/.nitro/vite/services/ssr/assets/router-DgvUVDCW.js
...
var styles_default = "/assets/styles-DGxZ04h7.css";
...
var $$splitComponentImporter$1 = () => import("./routes-DEnVaVOB.mjs");
var Route$5 = createFileRoute("/")({ component: lazyRouteComponent($$splitComponentImporter$1, "component") });
var $$splitComponentImporter = () => import("./login-CXDUw_Gz.mjs");
...
var Route$3 = createFileRoute("/api/grok")({ server: { handlers: { POST: async ({ request }) => {
	try {
		...
		if (action === "chatStream") {
			const { createGrokChatSseStream } = await import("./api-handlers-Bt8AST8P.mjs");
		...
		const { dispatchApi } = await import("./api-handlers-Bt8AST8P.mjs");
...
function isLocalRequest(request) {
	...
	if (h.startsWith("127.0.0.1") || h.startsWith("localhost") || h.startsWith("[::1]")) return true;
	return false;
}
function authorizeHost(request, body) {
	if (!isLocalRequest(request)) return "Host API is loopback-only";
...
var Route$2 = createFileRoute("/api/host")({ server: { handlers: { POST: async ({ request }) => {
	try {
		...
		if (required && [
			...
			"writeFile",
			"openApp"
		].includes(action) && !(request.headers.get("x-grokhub-host-token") === required || body.hostToken === required)) return Response.json({ error: "Host token required for this action" }, { status: 403 });
		const { dispatchHost } = await import("./host-api-handlers-C1lp0pF7.mjs");
