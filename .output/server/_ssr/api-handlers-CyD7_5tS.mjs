import { t as APP_VERSION, v as parseRateLimitHeaders } from "./version-RzUVPQXg.mjs";
import { XAI_BASE, callXaiChat, callXaiChatStream, callXaiImagine, probeXaiBearer } from "./grok-DjkVIlRC.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/api-handlers-CyD7_5tS.js
/**
...
var XAI_UA = "GrokHub/0.2.10 (xAI OAuth; Linux)";
...
async function dispatchApi(route, action, body) {
	if (route === "grok") {
		...
		if (action === "websiteUsage") {
			const { fetchGrokWebsiteUsageDirect } = await import("./grok-website-usage-CUFrqYHh.mjs").then((n) => n.r).then((n) => n.r);
		...
		if (action === "classifyModels") {
			...
			const { buildClassifyPrompt, parseGrokSlotPlan, pickSlotModel } = await import("./version-RzUVPQXg.mjs").then((n) => n.T).then((n) => n.w);
			let accessToken = body.accessToken ? String(body.accessToken) : void 0;
			const apiKey = body.apiKey ? String(body.apiKey) : void 0;
			if (body.tokens && typeof body.tokens === "object") try {
				accessToken = (await ensureAccessToken(body.tokens)).accessToken;
			} catch {}
			const classifierModel = pickSlotModel("fast", models) || "grok-4-1-fast-non-reasoning";
			const result = await callXaiChat({
				messages: [{
					role: "user",
					content: buildClassifyPrompt(models)
				}],
				mode: "fast",
		...
		if (action === "imagine") {
			...
			return callXaiImagine({
				...
				accessToken,
				apiKey,
				model: body.model ? String(body.model) : void 0,
				aspect: body.aspect ? String(body.aspect) : void 0,
				quality: body.quality === "quality" || body.quality === "speed" ? body.quality : void 0,
				mediaKind: body.mediaKind === "video" ? "video" : "image",
				n: typeof body.n === "number" ? body.n : void 0,
				referenceDataUrl: body.referenceDataUrl ? String(body.referenceDataUrl) : void 0
			});
		}
